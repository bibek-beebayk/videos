...
class ContributionType(models.Model):
    ...
    # contributor = models.ForeignKey('Contributor', related_name='contributions', on_delete=models.SET_NULL, null=True)
...
class Award(models.Model):
    ...
    media = models.ForeignKey('MediaBase', on_delete=models.SET_NULL, null=True, related_name='Media')
...
class MediaBase(models.Model):
    ...
    published_date = models.DateField(_("Published Date"), default=timezone.now() - datetime.timedelta(days=365))
    agency = models.CharField(max_length=256, null=True, blank=True,unique=True, db_index=True, verbose_name=_("Agency") )
    tags = models.ManyToManyField(Tag, related_name='media', blank=True, verbose_name=_("Tags"))
    media_type = models.ForeignKey(MediaType, on_delete=models.SET_NULL, null=True, related_name='media', verbose_name=_("Media Type"))
    company = models.ForeignKey(Company, on_delete=models.SET_NULL, null=True, related_name='media', verbose_name=_("Company"))
    commodities = models.ManyToManyField(Commodity, related_name='media', verbose_name=_("Commodities"))
    situations = models.ManyToManyField(Situation, related_name='media')
    views = models.PositiveIntegerField(default=0, blank=True, null=True, verbose_name=_("Views"))
...
class Video(MediaBase):
    video_file = models.FileField(_("Full Video"), upload_to='videos/', null=True, blank=True, validators=[FileExtensionValidator(allowed_extensions=['mp4', 'mov'])])
    genres = models.ManyToManyField(Genre, related_name='media', verbose_name=_("Genre"))
